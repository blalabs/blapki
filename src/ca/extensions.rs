...
mod tests {
    ...
    fn csr_with_san(san: Option<&str>) -> CertReq {
        let key = rsa::RsaPrivateKey::new(&mut rand::thread_rng(), 2048).unwrap();
        let signer = SigningKey::<Sha256>::new(key);
        let mut builder = RequestBuilder::new(Name::from_str("CN=dev").unwrap(), &signer).unwrap();
        if let Some(dns) = san {
            let gn = GeneralName::DnsName(der::asn1::Ia5String::new(dns).unwrap());
            builder.add_extension(&SubjectAltName(vec![gn])).unwrap();
        }
        builder.build::<rsa::pkcs1v15::Signature>().unwrap()
    }

    #[test]
    fn key_usage_maps_names() {
        let ku = key_usage(&["digitalSignature".into(), "keyEncipherment".into()]).unwrap();
    ...
    #[test]
    fn key_usage_rejects_unknown_name() {
        assert!(key_usage(&["telepathy".into()]).is_err());
    }

    #[test]
    fn eku_maps_names_and_raw_oids() {
        let eku = extended_key_usage(&["clientAuth".into(), "1.3.6.1.5.5.7.3.99".into()]).unwrap();
        assert_eq!(eku.0.len(), 2);
        assert!(eku.0.contains(&const_oid::db::rfc5280::ID_KP_CLIENT_AUTH));
    }

    #[test]
    fn eku_rejects_invalid_oid() {
        assert!(extended_key_usage(&["definitely not an oid".into()]).is_err());
    }

    #[test]
    fn requested_san_extracted_from_csr() {
        let csr = csr_with_san(Some("device.example.com"));
        assert!(requested_san(&csr).unwrap().is_some());
    }

    #[test]
    fn requested_san_absent_is_none() {
        let csr = csr_with_san(None);
        assert!(requested_san(&csr).unwrap().is_none());
    }
}

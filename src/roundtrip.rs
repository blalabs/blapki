...
#[test]
fn signature_by_a_different_key_than_the_embedded_cert_is_rejected() {
    let a = CertificateAuthority::generate_self_signed("a", "a", 2048).unwrap();
    let b = CertificateAuthority::generate_self_signed("b", "b", 2048).unwrap();
    let attrs = vec![
        attr::printable_attribute(attr::ID_MESSAGE_TYPE, MessageType::CertRep.as_value()).unwrap(),
        attr::printable_attribute(attr::ID_PKI_STATUS, PkiStatus::Failure.as_value()).unwrap(),
        attr::printable_attribute(attr::ID_FAIL_INFO, FailInfo::BadRequest.as_value()).unwrap(),
    ...
    // Sign with b's key but embed a's certificate: verification uses a's public
    // key and must reject the signature.
    let der = sign::build_cert_rep(&b.signing_key, &a.cert, None, attrs).unwrap();
    assert!(verify::parse_and_verify(&der).is_err());
}
